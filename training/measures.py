...
class Random(object):
    ...
    def __call__(self, delta_means, delta_vars):
        ...
        randoms = torch.randn(n_candidates, dtype=torch.float64).to(delta_means.device)
...
class Variance(object):
    ...
    def __call__(self, delta_means, delta_vars):
        ...
        variance = (
            torch.zeros(plan_horizon, n_candidates, dtype=torch.float64).to(delta_means.device)
        )
