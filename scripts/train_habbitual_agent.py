...
class ReplayBuffer:
    def __init__(self, env, max_episode_steps, buffer_size, sample_func, device):
        ...
        self.log_prob_memory = np.empty([self.size, self.max_episode_steps, self.action_dim], dtype=np.float32)
...
class Agent:
    def __init__(self, env, config):
        ...
        self.transition_net = MLP(self.state_size + self.action_dim,
                                  [256, 256], self.state_size, lr=0.001,
                                  device=self.device)
    ...
    def compute_value_net_loss(self, state_batch_t1, state_batch_t2,
        ...
        with torch.no_grad():
            ...
            weighted_targets = (log_prob_batch_t2.exp() * target_expected_free_energies_batch_t2).sum(-1).unsqueeze(1)
        ...
        efe_batch_t1 = (log_prob_batch_t1 * value_net_output).sum(-1).unsqueeze(1)

        # Determine the MSE loss between the EFE estimates and the value network output:
        value_net_loss = 0.5 * F.mse_loss(expected_free_energy_estimate_batch, efe_batch_t1)
        return value_net_loss

    def compute_variational_free_energy(self, state_batch_t1, pred_error_batch_t0t1):

        pres_actions_t1, pred_log_prob_t1 = self.actor.action_log_prob(state_batch_t1)

        value_net_input = torch.cat([state_batch_t1, pres_actions_t1], dim=1)
        ...
        energy_batch = (pred_log_prob_t1 * torch.log(boltzmann_expected_free_energy_batch_t1)).view(self.batch_size, 1)

        # Determine the entropy of the action distribution
        entropy_batch = -(pred_log_prob_t1 * torch.log(pred_log_prob_t1)).mean(-1).view(self.batch_size, 1)
