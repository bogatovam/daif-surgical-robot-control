...
class Agent:
    def __init__(self, argv):
        ...
        self.obs_shape = np.add(self.env.observation_space['observation'].shape,
    ...
    def set_parameters(self, argv):
        ...
        if self.keep_log:  # If true: write a message to the log
            self.record = open(self.log_path, "w+")
            self.record.write("\n\n-----------------------------------------------------------------\n")
    ...
    def train(self):
        ...
        for ith_episode in range(self.n_episodes):
            ...
            if ith_episode > 0 and ith_episode % self.print_timer == 0:
                ...
                msg = "Episodes: {:4d}, avg score: {:3.2f}, over last {:d}: {:3.2f}".format(
                    ith_episode, avg_reward, self.print_timer, last_x)
                print(msg)
            ...
            if self.save_network and ith_episode > 0 and ith_episode % self.network_save_timer == 0:
                torch.save(
                    self.transition_net.state_dict(),
        ...
        if self.save_network:
            ...
            torch.save(
                self.policy_net.state_dict(), "networks/intermediary/intermediary_polnet{}_end.pth".format(self.run_id)
            )
            torch.save(
                self.value_net.state_dict(), "networks/intermediary/intermediary_valnet{}_end.pth".format(self.run_id))
        ...
        if self.keep_log:
            self.record.write("Training finished at {}".format(datetime.datetime.now()))
            self.record.close()
