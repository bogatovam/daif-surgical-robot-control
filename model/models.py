...
class EnsembleDenseLayer(nn.Module):
    ...
    def reset_parameters(self):
        weights = torch.zeros(self.ensemble_size, self.in_size, self.out_size, dtype=torch.float64)
        biases = torch.zeros(self.ensemble_size, 1, self.out_size, dtype=torch.float64)
...
class RewardModel(nn.Module):
    ...
    def reset_parameters(self):
        self.fc_1 = nn.Linear(self.in_size, self.hidden_size, dtype=torch.float64)
        self.fc_2 = nn.Linear(self.hidden_size, self.hidden_size, dtype=torch.float64)
        self.fc_3 = nn.Linear(self.hidden_size, 1, dtype=torch.float64)
        self.to(self.device)
